...
namespace ff
{
	...
	auto TextureLoader::load(const std::string& path, const unsigned char* dataIn, uint32_t widthIn,
	                         uint32_t heightIn) -> Texture::Ptr
	{
		...
		else
		{
			...
			else
			{
				...
				bits = stbi_load_from_memory(dataIn, dataInSize, &width, &height, &picType,
