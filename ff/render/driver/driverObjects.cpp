...
namespace ff
{
	DriverObjects::Ptr DriverObjects::create(const DriverGeometries::Ptr& geometries,
	                                         const DriverAttributes::Ptr& attributes, const DriverInfo::Ptr& info)
	{
	...
	auto DriverObjects::update(const RenderableObject::Ptr& object) noexcept -> Geometry::Ptr
	{
		...
		if (const auto iter = mUpdateMap.find(geometry->getID()); iter == mUpdateMap.end()
